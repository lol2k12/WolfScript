--[[
=====================================================================
               WOLF CONTROL V41 - SANITY 120 HARD LOCK + MANUAL SAFE STAMP + FULL AUTO STAMP + FAST SAFE AUTO SURGERY + AUTO COLOR MEMORY / REGISTER-SAFE / DESKTOP MENU ON MOBILE / FIXED SMALL ALERTS / RELIABLE KEY CHARACTER ALERTS / SINGLE INSTANCE
                    English / Русский
=====================================================================
...
-- V40: phones/tablets use the EXACT desktop menu layout.
-- IS_TOUCH remains true for touch input and performance tuning only.
local UI_TOUCH = false
...
Toast.Size = IS_TOUCH and UDim2.fromOffset(300, 38) or UDim2.fromOffset(380, 48)
Toast.Position = UDim2.new(0.5, 0, 0, IS_TOUCH and 82 or 88)
Toast.BackgroundColor3 = C.CARD
Toast.ZIndex = 500
Toast.Visible = false
Toast.ClipsDescendants = false
Toast.Parent = GUI
corner(Toast, IS_TOUCH and 9 or 11)
local ToastStroke = stroke(Toast, C.CYAN, 1.2, 0.16)

local ToastSizeConstraint = Instance.new("UISizeConstraint")
ToastSizeConstraint.MinSize = Vector2.new(IS_TOUCH and 250 or 340, IS_TOUCH and 36 or 46)
ToastSizeConstraint.MaxSize = Vector2.new(IS_TOUCH and 320 or 420, IS_TOUCH and 42 or 54)
ToastSizeConstraint.Parent = Toast

local ToastIcon = Instance.new("TextLabel")
ToastIcon.Size = UDim2.fromOffset(IS_TOUCH and 32 or 40, IS_TOUCH and 38 or 48)
ToastIcon.BackgroundTransparency = 1
ToastIcon.Text = "🐺"
ToastIcon.TextSize = IS_TOUCH and 17 or 21
ToastIcon.ZIndex = 502
ToastIcon.Parent = Toast

local ToastText = Instance.new("TextLabel")
ToastText.Position = UDim2.fromOffset(IS_TOUCH and 31 or 40, 2)
ToastText.Size = UDim2.new(1, IS_TOUCH and -37 or -47, 1, -4)
...
ToastText.TextSize = IS_TOUCH and 10 or 12
...
local function toastVisibleY()
    ...
    -- Fixed safe zone below Roblox/game top HUD. No off-screen animation.
    if IS_TOUCH then
        return math.floor(math.clamp(viewport.Y * 0.105, 76, 104))
    end

    return math.floor(math.clamp(viewport.Y * 0.10, 78, 108))
end

local function hideToast(token)
    if token ~= nil and token ~= ToastToken then
        return
    end
...
local function notify(text, duration, important)
    ...
    -- Show directly at its final safe position so no part can be cut off.
    Toast.Position = UDim2.new(0.5, 0, 0, toastVisibleY())
    Toast.Visible = true

    task.delay(tonumber(duration) or (IS_TOUCH and 1.8 or 2.1), function()
...
local function notifyState(name, state, silent)
    ...
    notify(F(state and "N_ENABLED" or "N_DISABLED", {name = name}), IS_TOUCH and 1.35 or 1.8, false)
end
...
Icon.Name = "WolfIcon"
Icon.Size = UDim2.fromOffset(IS_TOUCH and 35 or 72, IS_TOUCH and 35 or 72)
Icon.Position = IS_TOUCH and UDim2.fromOffset(12, 138) or UDim2.new(0, 20, 0.36, 0)
Icon.BackgroundColor3 = C.CARD
Icon.Text = "🐺"
Icon.TextSize = IS_TOUCH and 17 or 36
Icon.TextColor3 = C.TEXT
Icon.AutoButtonColor = false
Icon.Parent = GUI
corner(Icon, IS_TOUCH and 11 or 23)
...
Glow.Size = UDim2.new(1, IS_TOUCH and 4 or 14, 1, IS_TOUCH and 4 or 14)
Glow.BackgroundTransparency = 1
Glow.ZIndex = 0
Glow.Parent = Icon
corner(Glow, IS_TOUCH and 13 or 27)
local GlowStroke = stroke(Glow, C.PURPLE, IS_TOUCH and 2 or 4, 0.48)
...
Main.Size = UDim2.fromOffset(BASE_DESKTOP.X, BASE_DESKTOP.Y)
...
local function updateScale()
    ...
    -- V40: identical 840x540 desktop UI on PC AND mobile.
    -- Mobile only scales the whole desktop UI uniformly if the viewport is
    -- physically too small. No alternate mobile layout is used.
    local marginX = IS_TOUCH and 18 or 30
    local marginY = IS_TOUCH and 18 or 30

    MainScale.Scale = math.min(
        1,
        math.max(0.38, (viewport.X - marginX) / BASE_DESKTOP.X),
        math.max(0.38, (viewport.Y - marginY) / BASE_DESKTOP.Y)
    )
end
...
Logo.Position = UDim2.fromOffset(20, 10)
Logo.Size = UDim2.fromOffset(47, 53)
...
Title.Position = UDim2.fromOffset(71, 10)
...
Version.Size = UDim2.new(1, -410, 0, 18)
...
Close.Position = UDim2.new(1, -54, 0, 17)
...
-- V40: desktop sidebar/content geometry is intentionally preserved on touch.
...
Scroll.Size = UDim2.new(1, -36, 1, -109)
...
local function createCard(height, iconText, titleText, description)
    ...
    local textX = 76
    local reserveRight = 230

    local title = Instance.new("TextLabel")
    title.Position = UDim2.fromOffset(textX, 13)
    title.Size = UDim2.new(1, -reserveRight, 0, 22)
    title.BackgroundTransparency = 1
    ...
    desc.Position = UDim2.fromOffset(textX, 36)
    desc.Size = UDim2.new(1, -reserveRight, 0, 22)
...
local function createToggle(iconText, titleText, description, getter, setter)
    ...
    status.Position = UDim2.new(1, -92, 0, 27)
    status.Size = UDim2.fromOffset(56, 18)
    ...
    switch.Size = UDim2.fromOffset(64, 32)
...
local function createSlider(iconText, titleText, description, minimum, maximum, getter, setter)
    ...
    value.Position = UDim2.new(1, -17, 0, 17)
    value.Size = UDim2.fromOffset(60, 28)
    ...
    bar.Position = UDim2.new(0, 18, 1, -24)
...
MainDragHandle.Size = UDim2.new(1, -205, 1, 0)
...
DragGrip.Position = UDim2.new(0.5, 0, 0, 61)
DragGrip.Size = UDim2.fromOffset(74, 14)
...
if not IS_TOUCH then
    notify(T("READY"), 1.35, false)
else
    Toast.Visible = false
end

-- V40 hard cleanup: the loading/READY toast must never remain stuck.
task.delay(2.0, function()
    if App.Active and ToastText.Text == T("READY") then
        Toast.Visible = false
    end
end)
